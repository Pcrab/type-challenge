...
type RemoveElem<T extends unknown[], U> = T extends [infer First, ...infer Rest]
    ? Equal<First, U> extends true
        ? RemoveElem<Rest, U>
        : [First, ...RemoveElem<Rest, U>]
    : [];

type Unique<T extends unknown[]> = T extends [infer First, ...infer Rest]
    ? [First, ...Unique<RemoveElem<Rest, First>>]
    : [];
...
export type cases = [
    ...
    Expect<
        ...
        >
    >,
    Expect<Equal<Unique<[unknown, unknown, never, never]>, [unknown, never]>>,
];
