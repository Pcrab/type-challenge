...
// 需要自己定义一个 PromiseLike 来满足 type T，只使用 builtin 的 Promise 无法满足需求
type PromiseLike<T> = {
    then: (onfulfilled: (value: T) => unknown) => unknown;
};

// 需要先限制传入的类型为 PromiseLike，来通过最后的错误判断
// 通过 infer 来获取实际传入的类型，并递归调用去除所有的 Promise
...
// @ts-expect-error MyAwaited only accept a promise like object
